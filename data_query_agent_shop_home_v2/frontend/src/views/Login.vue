<template>
  <main class="login-page" :class="{ admin: adminMode }">
    <button class="back" @click="$router.push('/')">← 返回淘气宝贝</button>
    <section class="login-shell">
      <div class="login-intro">
        <span class="mini-logo">{{ adminMode ? 'BI' : '淘' }}</span>
        <small>{{ adminMode ? 'ADMIN CONSOLE' : 'NAUGHTY BABY' }}</small>
        <h1>{{ adminMode ? '管理系统' : '欢迎回来' }}</h1>
        <p>{{ adminMode ? '库存、订单与智能数据分析工作台，仅限管理员账号访问。' : '登录淘气宝贝，继续挑选喜欢的商品并管理你的订单。' }}</p>
        <div class="mode-switch">
          <button v-if="adminMode" @click="$router.push('/login')">切换到用户登录</button>
          <button v-else @click="$router.push('/login?mode=admin')">管理员后台入口</button>
        </div>
      </div>
      <div class="login-panel">
        ...
        <el-form ref="loginFormRef" :model="loginForm" :rules="rules" label-position="top" @keyup.enter="login">
          ...
          <el-button class="login-button" type="primary" :loading="loading" @click="login">{{ adminMode ? '进入管理系统' : '登录淘气宝贝' }} <span>→</span></el-button>
        </el-form>
...
const login = async () => {
  ...
  try {
    ...
    if (response.data.role === 'admin') router.push(adminMode.value ? '/dashboard' : '/shop')
...
.login-page, .login-page.admin{background:radial-gradient(circle at 12% 8%,#89b8ae,#549688 48%,#315f56)}
.login-page.admin{background:radial-gradient(circle at 12% 8%,#6fa79b,#3f786e 48%,#244b44)}
.login-shell{border-color:#f4eac580;background:#f4eac524;box-shadow:0 40px 100px #173d3645}
.login-intro,.admin .login-intro{background:#244b4466}.admin .login-intro{background:#183d37a8}
.mini-logo,.admin .mini-logo{color:#3f786e;background:linear-gradient(135deg,#f4eac5,#dcebe7)}
.login-intro p,.admin .login-intro p{color:#f8f2db}
.mode-switch button{border-color:#f4eac580;background:#f4eac51a}.mode-switch button:hover{color:#315f56;background:#f4eac5}
.login-panel{background:#fffdf6}.panel-title small,.admin .panel-title small{color:#549688}.panel-title h2{color:#24413c}
.login-button,.admin .login-button{background:linear-gradient(90deg,#3f786e,#549688,#78aea2)}
.demo-account{color:#526e68;background:#f4eac5}.register-link button{color:#549688}
</style>
