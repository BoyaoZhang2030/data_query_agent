    ...
    <main class="shop-content">
      ...
      <section ref="productSection" class="products-section">
        ...
        <div v-loading="loading" class="product-grid">
          <article v-for="(product, index) in displayedProducts" :key="product.id" class="product-card">
            ...
            <div class="product-info">
              ...
              <div class="buy-line"><strong><small>¥</small>{{ formatPrice(product.price) }}</strong><el-button type="primary" round :disabled="product.stock <= 0" @click="addToCart(product)">加入购物车</el-button></div>
            </div>
          </article>
        </div>
...
const tones = ['#fff0e3', '#eaf5ff', '#f3edff', '#e9faf1', '#fff5d9', '#ffeef1']
...
const displayedProducts = computed(() => {
  ...
  if (appliedKeyword.value) {
    ...
    list = list.filter(p => `${p.name} ${p.description || ''}`.toLowerCase().includes(key))
  }
